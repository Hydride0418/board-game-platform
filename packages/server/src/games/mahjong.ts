import { IGameLogic, GameResult } from '@board-game/shared';

type Tile = string; // Using unicode characters directly
...
export class Mahjong implements IGameLogic {
    getInitialState() {
        // Shuffle deck
        ...
        return {
            ...
            lastDrawnTile: null as Tile | null, // Tile just drawn by current player
    ...
    processMove(gameState: any, move: any, playerId: string): { newState: any; result?: GameResult } {
        ...
        if (action === 'draw') {
            if (gameState.phase !== 'draw') throw new Error("Already drawn, please discard");
            
            const tile = gameState.deck.pop();
            if (!tile) return { newState: gameState, result: { isGameOver: true, winnerId: null } }; // Wall empty = Draw
        ...
        if (action === 'discard') {
             if (gameState.phase !== 'discard') throw new Error("Please draw a tile first");
             
             const hand = gameState.hands[playerId];
             if (!hand || !hand[tileIndex]) throw new Error("Invalid tile");
        ...
        if (action === 'reorder') {
            const { newHand } = move;
            if (!Array.isArray(newHand)) throw new Error("Invalid hand");
            
            // Validate that newHand is a permutation of the current hand
            const currentHand = gameState.hands[playerId] || [];
            if (newHand.length !== currentHand.length) throw new Error("Hand length mismatch");
    ...
    dealHands(deck: string[], playerIds: string[]) {
